...
public class BoidsView implements ChangeListener {
    ...
    public BoidsView(BoidsModel model, int width, int height, int nBoids) {
        ...
        resetButton.addActionListener(e -> {
           ...
           this.update(60);
            manager.tell(new ResetSimulation(this.model.getBoids()), ActorRef.noSender());
        });
        ...
        playButton.addActionListener(e -> {
            if (isRunning) {
                ...
                manager.tell(new StopSimulation(), ActorRef.noSender());
            } else {
                isRunning = true;
                playButton.setText("Suspend");
                nBoidsTextField.setForeground(Color.BLACK);
                resetButton.setEnabled(false);
                manager.tell(new StartSimulation(), ActorRef.noSender());
            }
        });
